...
評価指標の選択理由：MRRは複数正解を無視するため不適切（Q10のように複数ファイルが正解になるケースがある）。NDCGは重要度スコアの付与が必要で設計の前提と合わない。PrecisionとRecallはそれぞれ「無関係なファイルを取得していないか」「必要なファイルを取得できているか」という異なる側面を測るため両方を見る必要がある。F1はその調和平均であり、どちらかに極端に偏った戦略を低く評価できる点で今回の比較実験に適している。
...
| 指標 | スコア | 解釈 |
|---|---|---|
| Faithfulness | 0.400 | 非回答がスコアの平均を押し下げているとみられるが、回答が生成された質問では忠実度は高いと考えられる |
| Answer Relevancy | 0.180 | 「提供された文書には記載がありません」という非回答は質問との類似度が極めて低いため、全体スコアを大きく引き下げている |
